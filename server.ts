...
app.get('/api/distributors', authenticateRequest, async (req: any, res: any) => {
  ...
  async function resolveAuthSession(req: any): Promise<AuthenticatedUser | null> {
    ...
    // 3. Check for application role/email headers passed by authenticated client
    const userRoleHeader = (req.headers['x-user-role'] as string) || '';
    const userEmailHeader = (req.headers['x-user-email'] as string) || '';
    const userOrgHeader = (req.headers['x-user-org'] as string) || (req.headers['x-user-organization'] as string) || '';
    const userNameHeader = (req.headers['x-user-name'] as string) || '';
    if (userRoleHeader || userEmailHeader) {
      const role = userRoleHeader.toLowerCase().includes('distributor') ? 'Distributor' : userRoleHeader.toLowerCase().includes('admin') ? 'Admin' : 'Auditor';
      const email = userEmailHeader || (role === 'Distributor' ? 'distributor@midwest.com' : 'auditor@apex.com');
      const organization = userOrgHeader || (role === 'Auditor' ? 'Apex Audit Practice' : 'Midwest Trading Co.');
      const name = userNameHeader || (role === 'Distributor' ? 'Distributor User' : 'Senior Auditor');
      return {
  ...
  app.use('/api', async (req: any, res: any, next: any) => {
    // Skip public/auth and storage download/preview routes
  ...
  app.post('/api/storage/upload', upload.single('file'), authenticateRequest, async (req: any, res: any) => {
    try {
      ...
      // Accurately determine uploader role from request body or authenticated session
      const rawRoleCheck = String(req.body.uploaderRole || req.body.role || req.auth?.role || '').toLowerCase();
      const rawUserCheck = String(req.body.uploadedBy || req.auth?.name || req.auth?.email || '').toLowerCase();
      const isDistributorRole = req.auth?.role === 'Distributor' || rawRoleCheck.includes('distributor') || rawUserCheck.includes('distributor');
      ...
      let insertEvidenceRes: any = null;
      try {
        insertEvidenceRes = await supabase.from('system_audit_logs').insert({
          event_type: 'EVIDENCE_FILE',
          target_user_email: `${clientName}::${targetDistributor}`,
          details: newEvidenceRow,
          created_at: new Date().toISOString()
        }).select().single();
        if (insertEvidenceRes.error) insertError = insertEvidenceRes.error;
      } catch (err: any) {
        insertError = err;
      }

      // Resilient local store persistence backup
      try {
...
app.get('/api/sampling/questions', authenticateRequest, async (req: any, res: any) => {
  ...
  app.post('/api/sampling/upload', upload.single('file'), authenticateRequest, async (req: any, res: any) => {
    try {
      ...
      try {
        ...
        console.warn('Supabase sampling insert warning:', err);
      }
  ...
  // ====================================================================
  // Reports API (CRUD) - Supabase Persisted
  // ====================================================================
  ...
  app.post('/api/reporting/finalize', authenticateRequest, async (req: any, res: any) => {
    try {
      ...
      const { error } = await supabase.from('audit_reports').update({
        status: 'FINAL',
        report_version: '1.0',
        finalized_by: userEmail || req.auth?.email || 'Auditor',

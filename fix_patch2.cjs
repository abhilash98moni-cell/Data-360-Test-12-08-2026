...
const search = `        const { data: stateLogs } = await supabase.from('system_audit_logs').select('*').eq('event_type', 'IRL_DISTRIBUTOR_STATE');
...
const replace = `        // Fetch ordered by latest first to ensure we modify the active state
